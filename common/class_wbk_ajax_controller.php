...
class WBK_Ajax_Controller {
	...
	public function	ajaxCancelAppointment(){
		...
		$appointment_id = WBK_Db_Utils::getAppointmentIdByToken( $app_token );
	...
	protected function render_booking_form( $service_id, $time ){
		...
		if ( $service->getQuantity() > 1 ) {
			...
			$html .= '<label class="wbk-input-label" for="wbk-quantity">' . get_option( 'wbk_book_items_quantity_label', '' ) . '</label>';
